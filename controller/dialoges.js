...
const { updateLastSelection } = require("../model/file_helper");
const { logMessage, getDialogType, circularStringify } = require("../model/helper");
...
const getAllDialogs = async (client, sortByName = true) => {
    ...
    fs.writeFileSync(
        "../downloads/raw_dialog_list.json",
    ...
    fs.writeFileSync("../downloads/dialog_list.json", JSON.stringify(dialogList, null, 2));
    return dialogList;
}
...
const getDialogName = (channelId) => {
    try {
        let dialogs = require("../downloads/dialog_list.json")
        if (dialogs) {
